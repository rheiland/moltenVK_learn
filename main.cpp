...
//	device_layers.push_back( "VK_LAYER_LUNARG_standard_validation" );			// depricated
	...
	if( GLFW_FALSE == glfwVulkanSupported() ) {
		// not supported
		std::cout <<"-------- error glfwVulkanSupported == False " << std::endl;
		glfwTerminate();
		return -1;
	}
	...
	if( graphics_queue_family == UINT32_MAX ) {
		...
		std::cout <<"-------- error queue family not found " << std::endl;
		glfwTerminate();
		return -1;
	}
...
//	device_create_info.ppEnabledLayerNames			= device_layers.data();				// depricated
	...
	if( VK_SUCCESS != ret ) {
		...
		std::cout <<"-------- error from glfwCreateWindowSurface: " << ret << std::endl;
		glfwTerminate();
		return -1;
	}
	std::cout <<"-------- from glfwCreateWindowSurface: " << ret << std::endl;
